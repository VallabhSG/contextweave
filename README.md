...
| Proactive nudges | `GET /api/digest` — cached daily digest of focus, commitments, and slipping threads, surfaced in the UI — and **pushed by email**: opt in via `POST /api/digest/subscribe` (or the Space panel) and an hourly scheduler mails your nudge at your chosen hour, with one-click unsubscribe. Enable with `CW_RESEND_API_KEY` (required on HF Spaces — SMTP egress is blocked there) or `CW_SMTP_*` when self-hosting, plus `CW_PUBLIC_BASE_URL` |
